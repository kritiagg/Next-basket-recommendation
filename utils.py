...
def batchify(data, batch_size, is_reordered = False):
    ...
    if is_reordered is True:
        ...
        #Kriti's awesome brain. padding made easy :D
        #GoogleBrain <<< Kriti's excess gray cells. Google sad :'( , Kriti Happy :) :D
        num_batch = len(data) // batch_size
        for i in range(num_batch):
...
def pool_max(tensor, dim):
    ...
    return torch.max(tensor, dim, keepdim=True)[0]

def pool_avg(tensor, dim):
    return torch.mean(tensor, dim, keepdim=True)[0]
